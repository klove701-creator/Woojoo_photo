...
export class StorageManager {
  ...
  async logActivity(activityData) {
    if (this.firebaseOn && this.db) {
      return this.logActivityToFirebase(activityData);
    } else {
      return this.logActivityToLocal(activityData);
    }
  }

  // Firebase에 활동 로그 저장
  async logActivityToFirebase(activityData) {
    try {
      await addDoc(collection(this.db, 'activity-logs'), activityData);
      console.log('Firebase에 활동 로그 저장 성공');
    } catch (error) {
      console.error('Firebase 활동 로그 저장 오류:', error);
      // 로그 저장 실패는 치명적이지 않으므로 에러를 던지지 않음
    }
  }

  // 로컬에 활동 로그 저장
  logActivityToLocal(activityData) {
    try {
      const logs = JSON.parse(localStorage.getItem('activityLogs') || '[]');
      logs.unshift(activityData); // 최신 순으로
      if (logs.length > 100) logs.splice(100); // 최대 100개만 보관
  ...
  async loadActivityLogsFromFirebase() {
    try {
      const q = query(
        collection(this.db, 'activity-logs'), 
